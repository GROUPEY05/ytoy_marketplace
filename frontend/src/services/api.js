...
apiClient.interceptors.request.use(
  (config) => {
    ...
    if (token) {
      config.headers['Authorization'] = `Bearer ${token}`;  // Ajouter le jeton dans l'en-tête Authorization
    }
    return config;
  },
  (error) => {
...
const authService = {
  ...
  login: async (credentials) => {
    try {
      const response = await apiClient.post('/login', credentials);
      if (response.data.access_token) {
        localStorage.setItem('token', response.data.access_token);  // Stocker le jeton d'accès
        localStorage.setItem('user', JSON.stringify(response.data.utilisateur));  // Stocker l'utilisateur
        localStorage.setItem('userRole', response.data.utilisateur.role);  // Stocker le rôle de l'utilisateur
      }
      return response.data;
    } catch (error) {
      throw error.response?.data || { message: 'Erreur lors de la connexion' };
