    ...
    try {
      ...
      data.forEach(product => {
        ...
        if (product.images && product.images.length > 0) {
          ...
          const isAbsolute = product.images[0].url.startsWith('http://') || product.images[0].url.startsWith('https://') || product.images[0].url.startsWith('/');
          console.log('Is URL absolute?', isAbsolute);
          // Afficher l'URL complète qui sera utilisée
          const fullUrl = isAbsolute ? product.images[0].url : `${process.env.REACT_APP_API_URL}${product.images[0].url}`;
      ...
      const transformedData = data.map(product => ({
        ...
        images: product.images?.map(image => ({
          ...image,
          url: image.url.startsWith('http://') || image.url.startsWith('https://') || image.url.startsWith('/') 
            ? image.url 
      ...
      setError('Impossible de charger vos produits. Veuillez réessayer plus tard.');
    } finally {
      setLoading(false);
    }
  };
          ...
          {products.map(product => (
            <Col key={product.id} xs={12} sm={6} md={4} lg={3} className="mb-4">
              ...
              {product.images && product.images[0] && (
                  <>
                    {console.log('Rendering image with URL:', product.images[0].url)}
                    <Card.Img
                      variant="top"
                ...
                <Card.Body className="d-flex flex-column">
                  <Card.Title>
                    {editingProduct?.id === product.id && editField === 'nom' ? (
                      <input
                        type="text"
                        ...
                        onKeyPress={(e) => e.key === 'Enter' && handleUpdate(product.id, 'nom')}
                        autoFocus
                      />
                    ) : (
                      <span onClick={() => startEditing(product, 'nom')}>{product.nom}</span>
                    )}
                  </Card.Title>
                  <Card.Text className="text-muted mb-2">
                    {editingProduct?.id === product.id && editField === 'description' ? (
                      <textarea
                        className="form-control"
                        value={editValue}
                        onChange={(e) => setEditValue(e.target.value)}
                        onBlur={() => handleUpdate(product.id, 'description')}
                        rows="3"
                        autoFocus
                      />
                    ) : (
                      <span onClick={() => startEditing(product, 'description')}>
                        {product.description.substring(0, 100)}...
                      </span>
                    )}
                  </Card.Text>
                  <Card.Text className="text-primary fw-bold mb-3">
                    {editingProduct?.id === product.id && editField === 'prix' ? (
                      <input
                        type="number"
                        className="form-control"
                        value={editValue}
                        ...
                        onKeyPress={(e) => e.key === 'Enter' && handleUpdate(product.id, 'prix')}
                        autoFocus
                      />
                    ) : (
                      <span onClick={() => startEditing(product, 'prix')}>{product.prix} FCFA</span>
                    )}
                  </Card.Text>
                  <Card.Text className="mb-2">
                    Stock: {editingProduct?.id === product.id && editField === 'quantite_stock' ? (
                      <input
                        type="number"
                        className="form-control d-inline-block w-auto"
                        value={editValue}
                        onChange={(e) => setEditValue(e.target.value)}
                        onBlur={() => handleUpdate(product.id, 'quantite_stock')}
                        onKeyPress={(e) => e.key === 'Enter' && handleUpdate(product.id, 'quantite_stock')}
                        autoFocus
                      />
                    ) : (
                      <span onClick={() => startEditing(product, 'quantite_stock')}>{product.quantite_stock}</span>
                    )}
                  </Card.Text>
