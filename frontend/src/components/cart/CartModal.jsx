...
const CartModal = () => {
  ...
  const [defaultImage, setDefaultImage] = useState('http://localhost:8000/assets/placeholder.png')
  ...
  const fetchCartItems = async () => {
    try {
      ...
      const produitsAvecImages = (response.data.produits || []).map(produit => {
        ...
        // Essayer différentes structures possibles pour trouver l'image
        if (produit.images && produit.images.length > 0) {
          if (typeof produit.images[0] === 'string') {
            // Si l'image est directement une chaîne de caractères
            imageUrl = `http://localhost:8000/storage/${produit.images[0]}`
          } else if (produit.images[0].url) {
            // Si l'image est un objet avec une propriété url
            imageUrl = `http://localhost:8000/storage/${produit.images[0].url}`
          }
          ...
          // Si le produit a une propriété image_url
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      setError('Erreur lors de la création de la commande: ' + (error.response?.data?.message || error.message))
    } finally {
      setLoading(false)
    }
  }
              ...
              <div key={item.id} className="cart-item">
                ...
                <div className="item-quantity">
                  <div className="quantity-controls">
                    ...
                    <span className="quantity-display">{item.pivot?.quantite || 1}</span>
                    <button 
                      type="button" 
                      className="quantity-btn" 
                      onClick={() => {
